...
function View:update()
	if self.lobbyListView.game.multiplayerModel.room then
		self.modal:close()
	end
end
...
function View:newGameModal()
	...
	local scene = self.lobbyListView.gameView.scene
	local multiplayer_model = self.lobbyListView.game.multiplayerModel

	local fonts = self.shared.fontManager
	local username = self.lobbyListView.game.configModel.configs.online.user.name or "???"
	...
	self.modal:addChild("startGame", Button({
		...
		onClick = function ()
			...
			if name ~= "" then
				multiplayer_model:createRoom(name, password)
			end
		end
	}))
