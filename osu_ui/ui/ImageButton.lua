...
function ImageButton:bindEvents()
	self.parent:bindEvent(self, "keyPresseed")
end
...
---@param event table
function ImageButton:keyPressed(event)
...
function ImageButton:drawAnimation()
	...
	local c = ui.lighten(self.color, self.alpha * self.animation * 0.3)
...
function ImageButton:draw()
	...
	if self.hoverImage then
		...
		gfx.setColor(c[1], c[2], c[3], c[4] * self.animation * self.alpha)
		gfx.draw(self.hoverImage)
		return
	end

	c = ui.lighten(self.color, self.alpha * self.animation * 0.3)
	gfx.setColor(c[1], c[2], c[3], c[4] * self.alpha)
	gfx.draw(self.idleImage)
end
